...
var svgloader = '<div class="loader" title="2"><svg width="80" height="60" viewBox="5 0 80 60"><path class="wave" fill="none" stroke="#313131" stroke-width="5" stroke-linecap="round" d="M 0 37.5 c 7.684299348848887 0 7.172012725592294 -15 15 -15 s 7.172012725592294 15 15 15 s 7.172012725592294 -15 15 -15 s 7.172012725592294 15 15 15 s 7.172012725592294 -15 15 -15 s 7.172012725592294 15 15 15 s 7.172012725592294 -15 15 -15 s 7.172012725592294 15 15 15 s 7.172012725592294 -15 15 -15 s 7.172012725592294 15 15 15 s 7.172012725592294 -15 15 -15 s 7.172012725592294 15 15 15 s 7.172012725592294 -15 15 -15 s 7.172012725592294 15 15 15 s 7.172012725592294 -15 15 -15"></svg></div>';

    
$('#bid_variable').exists(function() {
    
    $(this).click(function () {
        
    var email = $(this).data("email");
    var namefio = $(this).data("namefio");
 ...
 $('.add-proceedings').click(function() {
         var aut = $(this).data("namefio");
         var rep = $(this).data("work");
         var rid = $(this).data("pid");
         var peramalink = $(this).data("link");
        
    $('#proc-'+rid+' .proceedings-link').val(peramalink);    
    $('#proc-'+rid+' .proceedings-aut').val(aut);        
    $('#proc-'+rid+' .proceedings-work').val(rep);
    $('#proc-'+rid+' .proceedings-id').val(rid);
...
$('#fullprice').exists(function () {
    
    var cur = $('#fullprice').html();
    var count = $('#count').val();
        
    $('#ya_summary').val(cur*count);
    $('#blnc_price').val(cur*count);
    var cur = $('#fullprice').html(cur*count);
    
    $('#aj').change(function(){
       var price = $('#aj input:checked').val();
       var count = $('#count').val();
        
       $('#fullprice').html(price*count);
       $('#ya_summary').val(price*count);
       $('#blnc_price').val(price*count);
    });
    
});
  
$('#blancG').exists(function () {
    $("#blancG .submit").click(function () {

    var act =  $('#blancG').attr('action');
     
    var toserv = $('#blancG').serialize();
...
$(".part_filter-it :checkbox").click(function() {
   $("#eqlist .eqit").fadeOut(500);
   
   $(".part_filter-it :checkbox:checked").each(function() {
       $("." + $(this).val()).fadeIn(500);
   });
});
...
$('#featured_upload').change(function() {
    $(this).append(svgloader);
    $(this).submit();
});

    
$('#reports').exists(function() {
    var options = {
        valueNames: [ 'date', 'order', 'name', 'mail', 'titel', 'confirm', 'pos' ]
        };
    var userList = new List('reports', options);
});
    

$('.agenda_sider').exists(function () {
    $(".agenda_sider a").click(function () {
        
    $(".agenda_sider a").removeClass();
        
    var act = $(this).data('url');
        
    var posid = $(this).data('slug');
    var posdt = $(this).data('posdt');
     ...
     $.ajaxSetup({
        ...
        cache:false,
        url: act,
        data: {"section" : posid, "jour" : posdt },
         complete: function () {
                console.log('done');
            }
         });
     
   $('#report_wrap').prepend('<span class="load">'+svgloader+' Секундочку</span>'); 
   $('#report_wrap').load(act);
        
   $(this).addClass('active');  
        
var postURL = '?section='+posid+'&jour='+posdt;
var postTTL = '';  
...
$(document).on("click", ".timedit a", function(event){
    var act = $(this).parent().attr('action');
    var posid = $(this).parent().serialize();
   ...
   $(this).parent().prepend('<span class="load">'+svgloader+'</span>'); 
   $(this).parent().load(act);
...
$(document).on("click", ".poup_up", function(event){
 
    var posid = $(this).attr("href");
    var abstract = $(posid).html();
...
$(document).on("click", ".abstract_block", function(event){
    var posid = $(this).toggle();
    return false;
    });
